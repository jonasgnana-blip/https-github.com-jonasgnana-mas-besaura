...
export default async function AdminDashboard({
  ...
  const [stats, refreshToken, pixelConfig] = await Promise.all([
    adminGetStats(),
    getStoredRefreshToken(),
    adminGetSistemaConfig("fb_pixel_id"),
  ]);

  const gcalConnected = !!refreshToken;

  const cards = [
    { label: "Reservas confirmadas", value: stats.confirmadas, icon: <Calendar size={20} className="text-[#4A6741]" />, bg: "bg-[#4A6741]/8" },
    { label: "Pendientes de pago",   value: stats.pendientes,  icon: <Clock size={20} className="text-[#8B6914]" />,    bg: "bg-[#8B6914]/8" },
    { label: "Total reservas",       value: stats.total,       icon: <TrendingUp size={20} className="text-[#C4A882]" />, bg: "bg-[#C4A882]/15" },
    ...
    <div className="p-8">
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-5 mb-6">
        {cards.map(({ label, value, icon, bg }) => (
          <div key={label} className="bg-white rounded-2xl border border-[#E8DCC8] p-6">
            <div className={`w-10 h-10 rounded-xl ${bg} flex items-center justify-center mb-4`}>{icon}</div>
            <div className="text-3xl font-semibold text-[#2C1810] mb-1" style={{ fontFamily: "var(--font-playfair), Georgia, serif" }}>
              {value}
            </div>
      ...
      <div className="grid md:grid-cols-2 gap-5 mb-6">
        <GcalCard
          ...
          searchMsg={params.gcal === "error" ? (params.msg ?? "Error desconocido") : undefined}
        />
        <PixelCard initialValue={pixelConfig?.valor ?? ""} />
      </div>

      {/* Backup */}
      <div className="bg-white rounded-2xl border border-[#E8DCC8] p-6 mb-6 flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-xs text-[#2C1810]/50 mt-0.5">Descarga todos los datos (reservas, actividades, configuración)</div>
          </div>
        </div>
        <BackupButton />
      </div>
